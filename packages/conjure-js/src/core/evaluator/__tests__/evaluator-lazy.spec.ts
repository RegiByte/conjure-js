...
describe('delay / force', () => {
  ...
  it('delay? predicate', () => {
    ...
    expect(session.evaluate('(delay? 42)')).toMatchObject(cljBoolean(false))
    expect(session.evaluate('(delay? nil)')).toMatchObject(cljBoolean(false))
  })

  it('realized? is false before force, true after', () => {
    const session = freshSession()
    session.evaluate('(def d (delay 42))')
    expect(session.evaluate('(realized? d)')).toMatchObject(cljBoolean(false))
    session.evaluate('(force d)')
    expect(session.evaluate('(realized? d)')).toMatchObject(cljBoolean(true))
  })
  ...
  it('side effects run only once', () => {
    ...
    expect(session.evaluate('@counter')).toMatchObject(cljNumber(0))
    session.evaluate('(force d)')
    expect(session.evaluate('@counter')).toMatchObject(cljNumber(1))
    session.evaluate('(force d)')
    expect(session.evaluate('@counter')).toMatchObject(cljNumber(1))
  })
  ...
  it('force on non-delay returns the value unchanged', () => {
    const session = freshSession()
    expect(session.evaluate('(force 42)')).toMatchObject(cljNumber(42))
    expect(session.evaluate('(force nil)')).toMatchObject(cljNil())
    expect(session.evaluate('(force "hello")')).toMatchObject(
      cljString('hello')
    )
  })
  ...
  it('delay body can reference lexical scope', () => {
    ...
    expect(result).toMatchObject(cljNumber(15))
  })
})
...
describe('lazy-seq basics', () => {
  ...
  it('lazy-seq? predicate', () => {
    ...
    expect(session.evaluate('(lazy-seq? nil)')).toMatchObject(cljBoolean(false))
  })

  it('realized? works on lazy-seq', () => {
    const session = freshSession()
    session.evaluate('(def ls (lazy-seq (list 1 2)))')
    expect(session.evaluate('(realized? ls)')).toMatchObject(cljBoolean(false))
    session.evaluate('(first ls)')
    expect(session.evaluate('(realized? ls)')).toMatchObject(cljBoolean(true))
  })
  ...
  it('rest on lazy-seq', () => {
    ...
    expect(result).toMatchObject(cljList([cljNumber(2)]))
  })
...
describe('infinite sequences', () => {
  ...
  it('(take 3 (repeat :x))', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljKeyword(':x'), cljKeyword(':x'), cljKeyword(':x')])
    )
  })
})
...
describe('lazy composition', () => {
  ...
  it('(take 3 (filter even? (range))) — lazy filter over infinite range', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(0), cljNumber(2), cljNumber(4)])
    )
  })
  ...
  it('(take 3 (drop 5 (range))) — lazy drop over infinite range', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(5), cljNumber(6), cljNumber(7)])
    )
  })
  ...
  it('(take 4 (take-while #(< % 10) (range))) — lazy take-while', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(0), cljNumber(1), cljNumber(2), cljNumber(3)])
    )
  })
  ...
  it('(take 4 (keep #(when (even? %) (* % 10)) (range))) — lazy keep', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(0), cljNumber(20), cljNumber(40), cljNumber(60)])
    )
  })
  ...
  it('chained: (take 3 (map inc (filter even? (range))))', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(1), cljNumber(3), cljNumber(5)])
    )
  })
})
...
describe('side-effect ordering', () => {
  it('elements are computed on demand, not eagerly', () => {
    ...
    expect(materialize(session.evaluate('@log'))).toMatchObject(cljVector([]))
...
describe('destructuring with lazy seqs', () => {
  it('(let [[a b c] (map inc [10 20 30])] [a b c])', () => {
    ...
    expect(result).toMatchObject(
      cljVector([cljNumber(11), cljNumber(21), cljNumber(31)])
    )
  })
...
describe('reduce over lazy', () => {
  it('(reduce + 0 (take 5 (range)))', () => {
    ...
    expect(result).toMatchObject(cljNumber(10))
  })
...
describe('into/vec with lazy seqs', () => {
  ...
  it('(into [] (take 3 (iterate inc 10)))', () => {
    ...
    expect(result).toMatchObject(
      cljVector([cljNumber(10), cljNumber(11), cljNumber(12)])
    )
  })
...
describe('printing lazy seqs', () => {
  it('prints realized lazy-seq as list', () => {
    ...
    expect(result).toMatchObject(cljString('(0 1 2)'))
  })

  it('delay prints as #<Delay ...>', () => {
    const session = freshSession()
    expect(session.evaluate('(str (delay 42))')).toMatchObject(
      cljString('#<Delay pending>')
    )
    session.evaluate('(def d (delay 42))')
    session.evaluate('(force d)')
    expect(session.evaluate('(str d)')).toMatchObject(cljString('#<Delay @42>'))
  })
})
...
describe('map-indexed — lazy 2-arg form', () => {
  it('finite coll returns lazy seq of [index item] pairs', () => {
    const session = freshSession()
    const result = materialize(session.evaluate('(map-indexed vector [:a :b :c])'))
    expect(result).toMatchObject(
      cljList([
  ...
  it('is lazy — works over infinite range', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        ...
        cljVector([cljNumber(1), cljNumber(11)]),
        cljVector([cljNumber(2), cljNumber(12)]),
        cljVector([cljNumber(3), cljNumber(13)]),
      ])
    )
  })
...
describe('keep-indexed — lazy 2-arg form', () => {
  it('keeps non-nil results with index', () => {
    const session = freshSession()
    const result = materialize(
      session.evaluate('(keep-indexed (fn [i x] (when (even? i) x)) [:a :b :c :d :e])')
    )
    expect(result).toMatchObject(
      cljList([cljKeyword(':a'), cljKeyword(':c'), cljKeyword(':e')])
    )
  })
  ...
  it('is lazy — works over infinite range', () => {
    const session = freshSession()
    const result = materialize(
      session.evaluate('(take 3 (keep-indexed (fn [i x] (when (even? i) x)) (range)))')
    )
    expect(result).toMatchObject(
      cljList([cljNumber(0), cljNumber(2), cljNumber(4)])
    )
  ...
  it('nil results are excluded', () => {
    const session = freshSession()
    const result = materialize(
      session.evaluate('(keep-indexed (fn [i x] (when (odd? i) (* x 10))) [1 2 3 4])')
    )
    expect(result).toMatchObject(
      cljList([cljNumber(20), cljNumber(40)])
    )
  })
})
...
describe('interleave — lazy', () => {
  it('2-arg: interleaves two finite seqs', () => {
    const session = freshSession()
    const result = materialize(session.evaluate('(interleave [1 2 3] [:a :b :c])'))
    expect(result).toMatchObject(
      cljList([
        cljNumber(1), cljKeyword(':a'),
        cljNumber(2), cljKeyword(':b'),
        cljNumber(3), cljKeyword(':c'),
      ])
    )
  })
  ...
  it('stops at the shorter coll', () => {
    const session = freshSession()
    const result = materialize(session.evaluate('(interleave [1 2] [:a :b :c :d])'))
    expect(result).toMatchObject(
      cljList([cljNumber(1), cljKeyword(':a'), cljNumber(2), cljKeyword(':b')])
    )
  })
  ...
  it('is lazy — works with infinite seqs', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljNumber(0), cljNumber(100),
        cljNumber(1), cljNumber(101),
        cljNumber(2), cljNumber(102),
      ])
    )
  })
  ...
  it('3-arg: interleaves three seqs', () => {
    const session = freshSession()
    const result = materialize(session.evaluate('(interleave [1 2] [:a :b] ["x" "y"])'))
    expect(result).toMatchObject(
      cljList([
        cljNumber(1), cljKeyword(':a'), cljString('x'),
        cljNumber(2), cljKeyword(':b'), cljString('y'),
      ])
    )
  })
})
...
describe('interpose — lazy, no count on coll', () => {
  it('inserts separator between elements', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(1), cljString(','), cljNumber(2), cljString(','), cljNumber(3)])
    )
  })
  ...
  it('single element — no separator', () => {
    ...
    expect(result).toMatchObject(cljList([cljNumber(42)]))
  })
  ...
  it('is lazy — works with infinite seqs (no OOM from count)', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljNumber(0), cljNumber(0),
        cljNumber(1), cljNumber(0),
        cljNumber(2),
      ])
    )
  })
  ...
  it('transducer form works', () => {
    ...
    expect(result).toMatchObject(
      cljVector([
        cljNumber(1), cljKeyword(':sep'), cljNumber(2), cljKeyword(':sep'), cljNumber(3),
      ])
    )
  })
})
...
describe('partition-by — lazy', () => {
  it('partitions by pred value change', () => {
    const session = freshSession()
    const result = materialize(session.evaluate('(partition-by odd? [1 1 2 2 3])'))
    expect(result).toMatchObject(
      cljList([
        cljVector([cljNumber(1), cljNumber(1)]),
        cljVector([cljNumber(2), cljNumber(2)]),
  ...
  it('is lazy — take from infinite partition-by', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljVector([cljNumber(0)]),
        cljVector([cljNumber(1)]),
        cljVector([cljNumber(2)]),
      ])
    )
  })
  ...
  it('consecutive equal values grouped', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljVector([cljNumber(1), cljNumber(1), cljNumber(1)]),
        cljVector([cljNumber(2), cljNumber(2)]),
        cljVector([cljNumber(3)]),
      ])
    )
...
describe('nfirst, newline', () => {
  it('nfirst = (next (first x))', () => {
    ...
    expect(result).toMatchObject(cljList([cljNumber(2), cljNumber(3)]))
  })

  it('nfirst of single-element inner returns nil', () => {
    const session = freshSession()
    expect(session.evaluate('(nfirst [[1] [2 3]])')).toMatchObject(cljNil())
  })

  it('newline returns nil', () => {
    const session = freshSession()
    expect(session.evaluate('(newline)')).toMatchObject(cljNil())
  })
})
...
describe('with-redefs', () => {
  it('temporarily replaces a var value', () => {
    ...
    expect(session.evaluate('(greet)')).toMatchObject(cljString('hello'))
    const result = session.evaluate(
      '(with-redefs [greet (fn [] "hi")] (greet))'
    )
    expect(result).toMatchObject(cljString('hi'))
  ...
  it('multiple bindings all restored', () => {
    ...
    expect(result).toMatchObject(
      cljVector([cljNumber(10), cljNumber(20)])
    )
    expect(session.evaluate('(f)')).toMatchObject(cljNumber(1))
    expect(session.evaluate('(g)')).toMatchObject(cljNumber(2))
  })

  it('empty bindings just runs body', () => {
    const session = freshSession()
    expect(session.evaluate('(with-redefs [] 42)')).toMatchObject(cljNumber(42))
  })
})
...
describe('mapcat — lazy', () => {
  it('flattens one level over finite coll', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljNumber(1), cljNumber(1),
        cljNumber(2), cljNumber(2),
        cljNumber(3), cljNumber(3),
      ])
    )
  })
  ...
  it('is lazy — works over infinite seq without OOM', () => {
    ...
    expect(result).toMatchObject(
      cljList([
        cljNumber(0), cljNumber(0),
        cljNumber(1), cljNumber(10),
        cljNumber(2), cljNumber(20),
      ])
    )
  })
  ...
  it('empty inner lists produce no output', () => {
    ...
    expect(result).toMatchObject(
      cljList([cljNumber(2), cljNumber(4)])
    )
  })
})
...
describe('nfirst — edge cases', () => {
  it('nfirst of nil returns nil', () => {
    const session = freshSession()
    expect(session.evaluate('(nfirst nil)')).toMatchObject(cljNil())
  })

  it('nfirst of empty outer coll returns nil', () => {
    const session = freshSession()
    expect(session.evaluate('(nfirst [])')).toMatchObject(cljNil())
  })
...
describe('quasiquote ~@ splicing with lazy seqs', () => {
  it('splices a lazy seq into a list', () => {
    ...
    const result = materialize(
      session.evaluate('`(0 ~@(map inc [1 2 3]) 5)')
    )
    expect(result).toMatchObject(
      cljList([
        cljNumber(0),
  ...
  it('splices nil as empty (no elements)', () => {
    ...
    expect(result).toMatchObject(cljList([cljNumber(1), cljNumber(2)]))
  })
