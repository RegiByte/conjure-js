...
describe('docstrings and metadata', () => {
  describe('with-meta / meta', () => {
    ...
    it('meta returns the attached map', () => {
      ...
      expect(s.evaluate('(meta f)')).toMatchObject(
        v.map([[v.keyword(':doc'), v.string('my doc')]])
      )
    })
    ...
    it('meta returns nil for a function with no metadata', () => {
      ...
      expect(s.evaluate('(meta f)')).toMatchObject(v.nil())
    })
  ...
  describe('defn with docstring', () => {
    ...
    it('meta carries :arglists even when defn has no docstring', () => {
      ...
      expect(s.evaluate("(:doc (meta #'add))")).toMatchObject(v.nil())
    })

    it('defn with docstring still works as a normal function', () => {
      const s = freshSession()
      s.evaluate('(defn square "Squares a number." [x] (* x x))')
      expect(s.evaluate('(square 5)')).toMatchObject(v.number(25))
    })
  ...
  describe('doc macro', () => {
    ...
    it('doc returns nil', () => {
      ...
      expect(s.evaluate('(doc f)')).toMatchObject(v.nil())
    })
  })
  ...
  describe('arglists metadata', () => {
    ...
    it(':arglists contains all param vectors for multi-arity defn', () => {
      ...
      expect(s.evaluate("(:arglists (meta #'f))")).toMatchObject(
        v.vector([
          v.vector([v.symbol('x')]),
          v.vector([v.symbol('x'), v.symbol('y')]),
        ])
      )
    })
    ...
    it(':arglists includes & for variadic params', () => {
      ...
      expect(arglists).toMatchObject(
        v.vector([
          v.vector([v.symbol('x'), v.symbol('&'), v.symbol('rest')]),
        ])
      )
    })
    ...
    it(':arglists is present on defn with docstring', () => {
      ...
      expect(s.evaluate("(:arglists (meta #'f))")).toMatchObject(
        v.vector([v.vector([v.symbol('a'), v.symbol('b')])])
      )
    })
...
describe('pos? / neg? / zero?', () => {
  it.each([
    ...
    const s = freshSession()
    expect(s.evaluate(code)).toEqual(v.boolean(expected as boolean))
  })
...
describe('scientific notation literals', () => {
  it('evaluates a bare integer exponent: 1e10', () => {
    const s = freshSession()
    expect(s.evaluate('1e10')).toEqual(v.number(1e10))
  })
  ...
  it('arithmetic with scientific notation literals', () => {
    const s = freshSession()
    expect(s.evaluate('(+ 1e3 2e3)')).toEqual(v.number(3000))
  })
})
...
describe('metadata on collections', () => {
  ...
  it('meta returns the attached map from a vector', () => {
    ...
    expect(s.evaluate('(meta (with-meta [1 2 3] {:tag "vec"}))')).toMatchObject(
      v.map([[v.keyword(':tag'), v.string('vec')]])
    )
  })
  ...
  it('meta returns the attached map from a map', () => {
    ...
    expect(s.evaluate('(meta (with-meta {:a 1} {:source "db"}))')).toMatchObject(
      v.map([[v.keyword(':source'), v.string('db')]])
    )
  })
  ...
  it('meta returns the attached map from a list', () => {
    ...
    expect(s.evaluate("(meta (with-meta '(1 2 3) {:kind \"list\"}))")).toMatchObject(
      v.map([[v.keyword(':kind'), v.string('list')]])
    )
  })

  it('meta returns nil for a vector with no metadata', () => {
    const s = freshSession()
    expect(s.evaluate('(meta [1 2 3])')).toMatchObject(v.nil())
  })
...
describe('metadata on symbols', () => {
  it('with-meta attaches metadata to a quoted symbol', () => {
    ...
    expect(s.evaluate("(meta (with-meta 'foo {:tag \"sym\"}))")).toMatchObject(
      v.map([[v.keyword(':tag'), v.string('sym')]])
    )
  })

  it('meta returns nil for an unadorned quoted symbol', () => {
    const s = freshSession()
    expect(s.evaluate("(meta 'foo)")).toMatchObject(v.nil())
  })
})
...
describe('^ reader macro on collections', () => {
  it('^:keyword attaches boolean true metadata to a vector', () => {
    ...
    expect(s.evaluate('(meta ^:special [1 2 3])')).toMatchObject(
      v.map([[v.keyword(':special'), v.boolean(true)]])
    )
  })
  ...
  it('^:keyword attaches metadata to a map literal', () => {
    ...
    expect(s.evaluate('(meta ^:indexed {:a 1})')).toMatchObject(
      v.map([[v.keyword(':indexed'), v.boolean(true)]])
    )
  })
})
...
describe('vary-meta', () => {
  ...
  it('merges onto existing metadata', () => {
    ...
    expect(s.evaluate('(:a (meta (vary-meta v assoc :b 2)))')).toMatchObject(v.number(1))
    expect(s.evaluate('(:b (meta (vary-meta v assoc :b 2)))')).toMatchObject(v.number(2))
  })
...
describe('var source positions', () => {
  it('stamps :line and :column on a def', () => {
    ...
    expect(s.evaluate('(:line   (meta #\'foo))')).toMatchObject(v.number(1))
    expect(s.evaluate('(:column (meta #\'foo))')).toMatchObject(v.number(5))
  })
  ...
  it('applies lineOffset when provided', () => {
    ...
    expect(s.evaluate('(:line (meta #\'foo))')).toMatchObject(v.number(10))
  })
  ...
  it('applies colOffset on line 1 only', () => {
    ...
    expect(s.evaluate('(:column (meta #\'foo))')).toMatchObject(v.number(8))
    // second def is on line 2 — colOffset must NOT apply
    s.evaluate('(def bar 1)\n(def baz 2)', { colOffset: 3 })
    expect(s.evaluate('(:column (meta #\'bar))')).toMatchObject(v.number(8)) // line 1, col 5+3
    expect(s.evaluate('(:column (meta #\'baz))')).toMatchObject(v.number(5)) // line 2, col 5 (no offset)
  })

  it('stamps :line on second form in a multi-line evaluate', () => {
    const s = freshSession()
    s.evaluate('(def bar 1)\n(def baz 2)')
    expect(s.evaluate('(:line (meta #\'bar))')).toMatchObject(v.number(1))
    expect(s.evaluate('(:line (meta #\'baz))')).toMatchObject(v.number(2))
  })
  ...
  it('stamps :file when loadFile provides a filePath', () => {
    ...
    s.setNs('demo.math')
    expect(s.evaluate('(:file (meta #\'pi))')).toMatchObject(v.string('/src/demo/math.clj'))
    expect(s.evaluate('(:line (meta #\'pi))')).toMatchObject(v.number(2))
  })
  ...
  it('no :file when loadFile is called without a filePath', () => {
    ...
    s.setNs('demo.core')
    expect(s.evaluate('(:file (meta #\'x))')).toMatchObject(v.nil())
  })

  it('preserves ^:dynamic alongside position metadata', () => {
    const s = freshSession()
    s.evaluate('(def ^:dynamic *level* 0)')
    expect(s.evaluate('(:dynamic (meta #\'*level*))')).toMatchObject(v.boolean(true))
    expect(s.evaluate('(:line   (meta #\'*level*))')).toMatchObject(v.number(1))
  })
  ...
  it('updates position on re-def', () => {
    ...
    expect(s.evaluate('(:line (meta #\'x))')).toMatchObject(v.number(7))
  })
})
...
describe('alter-meta!', () => {
  it('adds a key to var metadata', () => {
    ...
    expect(s.evaluate('(:my-key (meta #\'x))')).toMatchObject(v.string('hello'))
  })
  ...
  it('returns the new metadata map', () => {
    ...
    expect(s.evaluate('(:k (alter-meta! #\'x assoc :k 42))')).toMatchObject(v.number(42))
  })
  ...
  it('f receives nil when var has no prior user metadata', () => {
    ...
    expect(s.evaluate('(meta #\'bare-x)')).toMatchObject(v.nil())
    // now f receives nil
    s.evaluate('(alter-meta! #\'bare-x (fn [m] (assoc (or m {}) :added true)))')
    expect(s.evaluate('(:added (meta #\'bare-x))')).toMatchObject(v.boolean(true))
  })
  ...
  it('extra args are forwarded to f', () => {
    ...
    expect(s.evaluate('(:a (meta #\'x))')).toMatchObject(v.number(1))
    expect(s.evaluate('(:b (meta #\'x))')).toMatchObject(v.number(2))
  })
  ...
  it('preserves existing meta keys while adding new ones', () => {
    ...
    expect(s.evaluate('(:dynamic (meta #\'*x*))')).toMatchObject(v.boolean(true))
    expect(s.evaluate('(:tag (meta #\'*x*))')).toMatchObject(v.keyword(':extra'))
  })
  ...
  it('setting meta to nil clears it', () => {
    ...
    expect(s.evaluate('(meta #\'x)')).toMatchObject(v.nil())
  })
  ...
  it('does not affect the var value', () => {
    ...
    expect(s.evaluate('x')).toMatchObject(v.number(42))
  })
  ...
  it('works on atoms', () => {
    ...
    expect(s.evaluate('(:tag (meta a))')).toMatchObject(v.keyword(':x'))
  })

  it('(meta atom) returns atom metadata', () => {
    const s = freshSession()
    s.evaluate('(def a (atom 99))')
    expect(s.evaluate('(meta a)')).toMatchObject(v.nil())
    s.evaluate('(alter-meta! a assoc :k 1)')
    expect(s.evaluate('(:k (meta a))')).toMatchObject(v.number(1))
  })
