...
describe('destructuring', () => {
  describe('vector destructuring in let', () => {
    ...
    it('fewer values than patterns yields nil', () => {
      ...
      expect(isEqual(result, v.vector([v.number(1), v.number(2), v.nil()]))).toBe(true)
    })

    it('more values than patterns ignores extras', () => {
      const session = freshSession()
      const result = session.evaluate('(let [[a] [1 2 3]] a)')
      expect(isEqual(result, v.number(1))).toBe(true)
    })
    ...
    it('deeply nested destructuring', () => {
      ...
      expect(isEqual(result, v.number(42))).toBe(true)
    })

    it('nil value destructures to all nil', () => {
      const session = freshSession()
      const result = session.evaluate('(let [[a b] nil] [a b])')
      expect(isEqual(result, v.vector([v.nil(), v.nil()]))).toBe(true)
    })
  ...
  describe('map destructuring in let', () => {
    ...
    it(':or evaluates expressions lazily', () => {
      ...
      expect(isEqual(result, v.number(3))).toBe(true)
    })
    ...
    it(':or does not evaluate default when key is present', () => {
      ...
      const result = session.evaluate(`
        (let [called (atom false)
          ...
          [x @called])
      `)
      expect(isEqual(result, v.vector([v.number(42), v.boolean(false)]))).toBe(true)
    })
    ...
    it(':or with :strs', () => {
      ...
      expect(isEqual(result, v.number(50))).toBe(true)
    })
  })
  ...
  describe('destructuring in fn params', () => {
    ...
    it('map destructuring in fn', () => {
      ...
      expect(isEqual(result, v.number(7))).toBe(true)
    })
    ...
    it('defn with map destructuring', () => {
      ...
      const result = session.evaluate(`
        ...
        (greet {:name "World"})
      `)
      expect(isEqual(result, v.string('Hello, World'))).toBe(true)
    })
  })
  ...
  describe('destructuring in loop/recur', () => {
      ...
      const result = session.evaluate(`
        (loop [[x & xs] [1 2 3] acc 0]
          (if (nil? x)
            ...
            (recur xs (+ acc x))))
      `)
      expect(isEqual(result, v.number(6))).toBe(true)
    })
    ...
    it('map destructuring in loop', () => {
      ...
      `)
      expect(isEqual(result, v.number(120))).toBe(true)
    })
  ...
  describe('qualified :keys', () => {
    ...
    it('qualified :keys with :or defaults', () => {
      ...
      `)
      expect(isEqual(result, v.number(99))).toBe(true)
    })
    ...
    it('qualified :keys in fn params', () => {
      ...
      `)
      expect(isEqual(result, v.string('Bob <bob@example.com>'))).toBe(true)
    })
  })
