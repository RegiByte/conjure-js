...
describe('str', () => {
  it.each([
    ...
    (code, expected) => {
      ...
      expect(result).toMatchObject(v.string(expected))
    }
  )
})
...
describe('println', () => {
  it('println should call output callback and return nil', () => {
    ...
    expect(session.evaluate('(println "Hello" "world")')).toMatchObject(v.nil())
    expect(outputs).toEqual(['Hello world\n'])

    expect(session.evaluate('(println 1 2 3)')).toMatchObject(v.nil())
    expect(outputs).toEqual(['Hello world\n', '1 2 3\n'])
  })

  it('println falls back to console.log when no output callback provided', () => {
    const spy = vi.spyOn(console, 'log').mockImplementation(() => {})
    expect(freshSession().evaluate('(println "test")')).toMatchObject(v.nil())
...
describe('eval', () => {
  ...
  ])('should evaluate eval core function %s should be %s', (code, expected) => {
    const session = freshSession()
    const result = session.evaluate(code)
...
describe('apply', () => {
  ...
  it('(apply map ...) returns a seq (list)', () => {
    ...
    expect(result).toMatchObject(
      v.list([v.number(2), v.number(3), v.number(4), v.number(5), v.number(6)])
    )
  })
})
  ...
  it.each([
    ...
    [
      ...
      v.map([
        ...
        [v.keyword(':b'), v.number(3)],
      ]),
    ],
    ['(merge nil {:a 1})', v.map([[v.keyword(':a'), v.number(1)]])],
...
describe('select-keys', () => {
  it.each([
    ['(select-keys {:a 1 :b 2} [:a])', v.map([[v.keyword(':a'), v.number(1)]])],
    ['(select-keys {:a nil :b 2} [:a])', v.map([[v.keyword(':a'), v.nil()]])],
    ['(select-keys nil [:a])', v.map([])],
    ['(select-keys {:a 1} nil)', v.map([])],
...
describe('update', () => {
  it.each([
    ...
    [
      ...
      v.map([
        [v.keyword(':a'), v.vector([v.number(1), v.number(2), v.number(3)])],
      ]),
    ],
    [
      '(update nil :a (fn [x] (if (nil? x) 0 x)))',
      v.map([[v.keyword(':a'), v.number(0)]]),
    ],
...
describe('frequencies', () => {
  it.each([
    [
      ...
      v.map([
        [v.number(1), v.number(2)],
        [v.number(2), v.number(2)],
        [v.number(3), v.number(1)],
      ]),
    ],
    ['(frequencies nil)', v.map([])],
...
describe('group-by', () => {
  it.each([
    [
      ...
      v.map([
        [v.boolean(true), v.vector([v.number(1), v.number(3)])],
        [v.boolean(false), v.vector([v.number(2), v.number(4)])],
      ]),
    ],
    ['(group-by inc nil)', v.map([])],
...
describe('reduce-kv', () => {
  ...
  it.each([["(reduce-kv + 0 '(1 2))", 'reduce-kv expects a map or vector']])(
...
describe('sort and sort-by', () => {
  ...
  ])('should evaluate sort/sort-by core functions: %s', (code, expected) => {
    const session = freshSession()
    const result = session.evaluate(code)
    expect(result).toMatchObject(toCljValue(expected))
  })
...
describe('comp', () => {
  ...
  ])('should evaluate comp: %s → %s', (code, expected) => {
    const session = freshSession()
    const result = session.evaluate(code)
    expect(result).toMatchObject(toCljValue(expected))
  })
    ...
    expect(result).toMatchObject(
      v.list([
        v.map([
          ...
          [v.keyword(':role'), v.keyword(':admin')],
        ]),
      ])
    )
  })
  ...
  it('(comp :role) as a standalone extractor', () => {
    ...
    expect(result).toMatchObject(v.string('Alice'))
  })

  it.each([['(comp 1 inc)', 'comp expects functions']])(
...
describe('partial', () => {
  ...
  ])('should evaluate partial: %s → %s', (code, expected) => {
    const session = freshSession()
  ...
  it.each([['(partial 42)', 'partial expects a callable']])(
