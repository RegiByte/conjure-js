...
describe('count', () => {
  it.each([
    ...
    (code, expected) => {
      ...
      expect(result).toMatchObject(v.number(expected))
    }
  )
...
describe('first', () => {
  it.each([
    ...
    ['(first [])', null],
    ["(first '())", null],
    ['(first {"a" 1 "b" 2})', [v.string('a'), v.number(1)]],
    ['(first [1 2])', 1],
    ["(first '(2 3))", 2],
    ['(first "hello")', 'h'],
    ['(first "")', null],
...
describe('conj', () => {
  it.each([
    ...
    ['(conj {"a" 1})', v.map([[v.string('a'), v.number(1)]])],
    ["(conj '(1 2))", v.list([v.number(1), v.number(2)])],
    // basic conj
    ...
    [
      "(conj '(1 2) 3 4)",
      v.list([v.number(4), v.number(3), v.number(1), v.number(2)]),
    ],
    [
      "(conj '(1 2) 3 4 5)",
      v.list([
...
(assoc db :users (cons new-element (:users db)))`,
      v.map([
        [
          v.keyword(':users'),
          v.vector([
            v.map([[v.keyword(':name'), v.string('Eva')]]),
            v.map([[v.keyword(':name'), v.string('Eduardo')]]),
          ]),
        ],
      ]),
    ],
...
describe('seq', () => {
  it.each([
    ['(seq [1 2 3])', v.list([v.number(1), v.number(2), v.number(3)])],
    ["(seq '(1 2 3))", v.list([v.number(1), v.number(2), v.number(3)])],
    [
      '(seq {:a 1 :b 2})',
      v.list([
        v.vector([v.keyword(':a'), v.number(1)]),
        v.vector([v.keyword(':b'), v.number(2)]),
      ]),
    ],
    [
      '(seq "hello")',
      v.list([v.string('h'), v.string('e'), v.string('l'), v.string('l'), v.string('o')]),
    ],
    ['(seq "a")', v.list([v.string('a')])],
    ['(seq "")', null],
    ['(seq [])', null],
    ["(seq '())", null],
    ['(seq nil)', null],
...
describe('strings as seqable', () => {
  it('reduce over a string', () => {
    ...
    expect(result).toMatchObject(v.string('hello'))
  })

  it('apply str over a string', () => {
    const session = freshSession()
    const result = session.evaluate('(apply str "abc")')
    expect(result).toMatchObject(v.string('abc'))
  })
  ...
  it('count on a string', () => {
    ...
    expect(result).toMatchObject(v.number(5))
  })

  it('unicode-safe seq', () => {
    const session = freshSession()
    const result = session.evaluate('(count (seq "café"))')
    expect(result).toMatchObject(v.number(4))
  })
})
...
describe('map', () => {
 ...
 [(str (get entry 0) (get entry 0)),
  (* 2 (get entry 1))])
   {:a 1 :b 2})`,
      v.list([
        v.vector([v.string(':a:a'), v.number(2)]),
        v.vector([v.string(':b:b'), v.number(4)]),
      ]),
    ],
    // nil collection returns empty seq
    ['(map (fn [n] (+ n 1)) nil)', v.list([])],
  ...
  describe('multi-collection (zip)', () => {
    it('(map + [1 2 3] [10 20 30]) applies f to corresponding items', () => {
      ...
      expect(s.evaluate('(map + [1 2 3] [10 20 30])')).toMatchObject(
        v.vector([v.number(11), v.number(22), v.number(33)])
      )
    })
    ...
    it('(map vector ...) zips two collections into pairs', () => {
      ...
      expect(s.evaluate('(map vector [1 2 3] [4 5 6])')).toMatchObject(
        v.vector([
          v.vector([v.number(1), v.number(4)]),
          v.vector([v.number(2), v.number(5)]),
          v.vector([v.number(3), v.number(6)]),
        ])
      )
    })
    ...
    it('4 collections zip correctly', () => {
      ...
      ).toMatchObject(v.vector([v.number(1111), v.number(2222)]))
    })
  })
})
...
describe('filter', () => {
  ...
  it.each([
    ...
    [
      ...
      v.list([v.number(3), v.number(4), v.number(5)]),
    ],
    [
      "(filter (fn [n] (> n 2)) '(1 2 3 4 5))",
      v.list([v.number(3), v.number(4), v.number(5)]),
    ...
    [
      ...
      v.list([v.string('b'), v.string('c'), v.string('d'), v.string('e')]),
    ],
    [
      `(filter (fn [entry] (> (get entry 1) 2)) {:a 1 :b 2 :c 3 :d 4})`,
      v.list([
        v.vector([v.keyword(':c'), v.number(3)]),
        v.vector([v.keyword(':d'), v.number(4)]),
      ]),
    ],
    // nil collection returns empty seq
    ['(filter (fn [n] (> n 2)) nil)', v.list([])],
...
describe('reduce', () => {
  it.each([
    ...
    [
     ...
     {:a 1 :b 2 :c 3})`,
      v.map([
        [v.keyword(':a'), v.number(2)],
        [v.keyword(':b'), v.number(4)],
        [v.keyword(':c'), v.number(6)],
      ]),
    ],
...
describe('nth', () => {
  it.each([
    ...
    ['(nth [10 20 30] 2)', 30],
    ["(nth '(10 20 30) 1)", 20],
    ['(nth [10 20 30] 5 :missing)', v.keyword(':missing')],
    ["(nth '() 0 :missing)", v.keyword(':missing')],
...
describe('take and drop', () => {
  ...
  it.each([
    ...
    ['(take 2 [1 2 3 4])', [v.number(1), v.number(2)]],
    ['(take 0 [1 2 3])', []],
    ['(take -1 [1 2 3])', []],
    ['(take 10 [1 2 3])', [v.number(1), v.number(2), v.number(3)]],
    ["(take 2 '(1 2 3 4))", [v.number(1), v.number(2)]],
    [
      '(take 2 {:a 1 :b 2 :c 3})',
      [
    ...
    ['(drop 2 [1 2 3 4])', [v.number(3), v.number(4)]],
    ['(drop 0 [1 2 3])', [v.number(1), v.number(2), v.number(3)]],
    ['(drop -1 [1 2 3])', [v.number(1), v.number(2), v.number(3)]],
    ['(drop 10 [1 2 3])', []],
    ["(drop 1 '(1 2 3))", [v.number(2), v.number(3)]],
  ])('should evaluate take / drop: %s → %s', (code, expected) => {
    const session = freshSession()
    const result = materialize(session.evaluate(code))
    expect(result).toMatchObject(
      v.list((expected as any[]).map(toCljValue))
    )
  })
...
describe('concat', () => {
  it.each([
    ...
    ["(concat '(1) [2] {:a 3})", [1, 2, [v.keyword(':a'), 3]]],
    [
      "(concat '(1) [2 {:a 3}])",
      [
        v.number(1),
        v.number(2),
        v.map([[v.keyword(':a'), v.number(3)]]),
      ],
    ],
    ['(concat [] [])', []],
    ...
    const session = freshSession()
    const result = materialize(session.evaluate(code))
    const expectedList = v.list((expected as any[]).map(toCljValue))
...
describe('into', () => {
  it.each([
    ...
    (code, expected) => {
      ...
      const result = session.evaluate(code)
      expect(result).toMatchObject(
        v.vector((expected as number[]).map(toCljValue))
      )
    }
  )
  ...
  it.each([
    ...
    (code, expected) => {
      ...
      const result = session.evaluate(code)
      expect(result).toMatchObject(
        v.list((expected as number[]).map(toCljValue))
      )
    }
  )
...
describe('zipmap', () => {
  ...
  it('should evaluate zipmap with empty collections', () => {
    const session = freshSession()
    expect(session.evaluate('(zipmap [] [])')).toMatchObject(v.map([]))
  })
...
describe('keys and vals', () => {
  it.each([
    ['(keys {:a 1 :b 2})', [v.keyword(':a'), v.keyword(':b')]],
    ['(keys {})', []],
    ['(vals {:a 1 :b 2})', [1, 2]],
    [
      '(vals {:a 1 :b 2 :c [1 2 3]})',
      [1, 2, v.vector([v.number(1), v.number(2), v.number(3)])],
    ],
    ['(vals {})', []],
    ...
    const result = session.evaluate(code)
    expect(result).toMatchObject(
      v.vector((expected as any[]).map(toCljValue))
    )
  })
