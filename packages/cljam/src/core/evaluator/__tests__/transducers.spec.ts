...
describe('reduced / unreduced / ensure-reduced', () => {
  ...
  it('(reduced? (reduced 42)) is true', () => {
    const s = freshSession()
    expect(s.evaluate('(reduced? (reduced 42))')).toEqual(v.boolean(true))
  })

  it('(reduced? 42) is false', () => {
    const s = freshSession()
    expect(s.evaluate('(reduced? 42)')).toEqual(v.boolean(false))
  })

  it('(unreduced (reduced 42)) unwraps to 42', () => {
    const s = freshSession()
    expect(s.evaluate('(unreduced (reduced 42))')).toEqual(v.number(42))
  })

  it('(unreduced 42) returns 42 unchanged', () => {
    const s = freshSession()
    expect(s.evaluate('(unreduced 42)')).toEqual(v.number(42))
  })

  it('@(reduced 42) unwraps via deref', () => {
    const s = freshSession()
    expect(s.evaluate('(deref (reduced 42))')).toEqual(v.number(42))
  })
  ...
  it('reduce short-circuits on reduced', () => {
    ...
    ).toMatchObject(v.vector([v.number(1), v.number(2)]))
  })
})
...
describe('volatile!', () => {
  ...
  it('(volatile? (volatile! 0)) is true', () => {
    const s = freshSession()
    expect(s.evaluate('(volatile? (volatile! 0))')).toEqual(v.boolean(true))
  })

  it('(volatile? 42) is false', () => {
    const s = freshSession()
    expect(s.evaluate('(volatile? 42)')).toEqual(v.boolean(false))
  })

  it('@v returns current value', () => {
    const s = freshSession()
    expect(s.evaluate('(let [v (volatile! 42)] @v)')).toEqual(v.number(42))
  })
...
describe('transduce', () => {
  it('(transduce (map inc) conj [] [1 2 3]) produces [2 3 4]', () => {
    ...
    expect(s.evaluate('(transduce (map inc) conj [] [1 2 3])')).toMatchObject(
      v.vector([v.number(2), v.number(3), v.number(4)])
    )
  })
  ...
  it('(transduce (filter even?) conj [] [1 2 3 4 5]) produces [2 4]', () => {
    ...
    ).toMatchObject(v.vector([v.number(2), v.number(4)]))
  })
  ...
  it('(transduce (remove even?) conj [] [1 3 5]) produces [1 3 5]', () => {
    ...
    ).toMatchObject(v.vector([v.number(1), v.number(3), v.number(5)]))
  })
  ...
  it('comp composes transducers left-to-right', () => {
    ...
    ).toMatchObject(v.vector([v.number(2), v.number(4), v.number(6)]))
  })
...
describe('into with transducer', () => {
  ...
  it('(into [] (filter odd?) [1 2 3 4 5]) keeps odds', () => {
    ...
    expect(s.evaluate('(into [] (filter odd?) [1 2 3 4 5])')).toMatchObject(
      v.vector([v.number(1), v.number(3), v.number(5)])
    )
  })
...
describe('take-while transducer', () => {
  it('(take-while pos? [1 2 0 3]) stops at 0', () => {
    const s = freshSession()
    expect(
      materialize(s.evaluate('(take-while pos? [1 2 0 3])'))
    ).toMatchObject(v.list([v.number(1), v.number(2)]))
  })
...
describe('drop-while transducer', () => {
  it('(drop-while neg? [-1 -2 3 4]) skips negatives', () => {
    const s = freshSession()
    expect(
      materialize(s.evaluate('(drop-while neg? [-1 -2 3 4])'))
    ).toMatchObject(v.list([v.number(3), v.number(4)]))
  })

  it('passes through everything once pred fails', () => {
    const s = freshSession()
    expect(
      materialize(s.evaluate('(drop-while even? [2 4 5 6])'))
    ).toMatchObject(v.list([v.number(5), v.number(6)]))
  })
})
...
describe('map-indexed transducer', () => {
  it('(map-indexed vector [10 20 30]) adds index', () => {
    const s = freshSession()
    expect(
      materialize(s.evaluate('(map-indexed vector [10 20 30])'))
    ).toMatchObject(
      v.list([
        v.vector([v.number(0), v.number(10)]),
        v.vector([v.number(1), v.number(20)]),
        v.vector([v.number(2), v.number(30)]),
      ])
    )
  })
  ...
  it('can compose with other transducers', () => {
    ...
    expect(
      ...
      v.vector([
        ...
        v.vector([v.number(5), v.number(7)]),
      ])
    )
  })
})
